...
Display( )
{
	...
	//glBindTexture( GL_TEXTURE_CUBE_MAP, CubeMapTex );
	//CubeMapShader.SetUniformVariable( "uReflectUnit", ReflectUnit );
	//CubeMapShader.SetUniformVariable( "uRefractUnit", RefractUnit );
...
InitGraphics( )
{
	...
	CubeMapShader.SetUniformVariable( "uColorMix", 0.25f );
 ...
 //   int width, height;
 //   unsigned char* texture = BmpToTexture(CubeFaceFiles[0], &width, &height);
	...
	for (int file = 0; file < 6; file++) {
		...
		if (texture2d == NULL) {
			fprintf(stderr, "Could not open BMP 2D texture '%s'\n", CubeFaceFiles[file]);
		}
		else {
