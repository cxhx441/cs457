...
{
	GLCall(GLint loc = glGetUniformLocation(program, name));
	if (loc < 0) 
...
{
	GLint loc = get_uniform_loc(program, name);
	if (count == 2)      { GLCall(glUniform2fv(loc, 1, val)); }
	else if (count == 3) { GLCall(glUniform3fv(loc, 1, val)); }
	else if (count == 4) { GLCall(glUniform4fv(loc, 1, val)); }
};

void
set_uniform_variable( GLuint program, const char* name, glm::mat3 val)
{
	GLint loc = get_uniform_loc(program, name);
...
void
set_uniform_variable( GLuint program, const char* name, glm::mat4 val)
{
	GLint loc = get_uniform_loc(program, name);
...
void
set_uniform_variable( GLuint program, const char* name, glm::vec3 val)
{
	GLint loc = get_uniform_loc(program, name);
...
void
set_uniform_variable( GLuint program, const char* name, glm::vec4 val)
{
	GLint loc = get_uniform_loc(program, name);
